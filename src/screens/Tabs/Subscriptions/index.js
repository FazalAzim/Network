...
const SubscribeCard = ({ data }) => {
  return (
    <TouchableOpacity>
      <Wrapper style={{ marginBottom: 14, position: 'relative', width: width(80), height: height(16), borderWidth: 1, borderColor: data.title === 'Yearly' ? COLORS.PRIMARY_COLOR : COLORS._C4C4, borderRadius: 18 }}>
        {data.button &&
        ...
        <Wrapper style={{ width: '100%', height: '100%', flexDirection: 'column', gap: 3, justifyContent: 'center', alignItems: 'center' }}>
          ...
          <RowWrapperBasic style={{ gap: 6 }}>
            <Text style={{ textAlign: 'center', fontSize: 14, fontFamily: FONTS.URBAN_MEDIUM }}>{`$${data.price}/month`}</Text>
            {'priceMonth' in data &&
    ...
    <MainWrapper style={{ paddingHorizontal: width(3) }}>
      ...
      <Wrapper style={{ width: width(80), alignSelf: 'center' }} >
        ...
        <FlatList
          data={data}
          renderItem={({ item }) => {
            return <SubscribeCard data={item} />;
          }}
        />
